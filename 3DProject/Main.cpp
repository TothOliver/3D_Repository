...
int APIENTRY wWinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPWSTR lpCmdLine, _In_ int nCmdShow)
{
	...
	if (!ShaderLoader(device, immediateContext, vertexShader, computeShader, ps[0], ps[1], inputLayout, sampler))
	{
		std::cerr << "Error: SetupD3D11" << std::endl;
		return -1;
	}
	...
	while (!(GetKeyState(VK_ESCAPE) & 0x8000) && msg.message != WM_QUIT)
	{
			...
			if (mesh->IsReflective())
			{
				...
				for (size_t side = 0; side < 6; side++)
				{
					RenderToTargetUAV(device, immediateContext, cubeMap->GetCubeUAV(side), cubeMap->GetCubeSideLength(), cubeMap->GetCubeSideLength(),
						cubeMap->GetCubeDSV(), inputLayout.GetInputLayout(), sampler.GetSamplerState(), cubeMap->GetCubeViewport(), cubeMap->GetCameraVPBuffer(side),
		...
		RenderToTargetUAV(device, immediateContext, uav, WIDTH, HEIGHT, dsView, inputLayout.GetInputLayout(), 
			sampler.GetSamplerState(), viewport, camera.GetVPBuffer(), camera.GetPositionBuffer(), scenes, sceneIndex, 0, ps);
