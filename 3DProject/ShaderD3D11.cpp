...
ShaderD3D11::ShaderD3D11(ID3D11Device* device, ShaderType shaderType, const void* dataPtr, size_t dataSize, const char* csoPath)
{
	...
	Initialize(device, shaderType, dataPtr, dataSize, csoPath);
}
...
void ShaderD3D11::Initialize(ID3D11Device* device, ShaderType shaderType, const void* dataPtr, size_t dataSize, const char* csoPath)
{
	...
	dataPtr = shaderData.c_str();
	dataSize = shaderData.length();
	shaderData.clear();
	reader.close();
}
