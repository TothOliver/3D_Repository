...
int APIENTRY wWinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPWSTR lpCmdLine, _In_ int nCmdShow)
{
	...
	CameraD3D11 camera;
	camera.Initialize(device, { 1, (float)16 / 9, 1, 20 }, { 0, 5, -10.0f });
	...
	matricies[1] = camera.GetViewProjectionMatrix();
	...
	while (!(GetAsyncKeyState(VK_ESCAPE) & 0x8000) && msg.message != WM_QUIT)
	{
		...
		//CameraMovement(camera, p, rect);
		camera.UpdateInternalConstantBuffer(immediateContext);

		ID3D11Buffer* camBuffer = camera.GetConstantBuffer();
		immediateContext->PSSetConstantBuffers(0, 1, &camBuffer);

		vsBuffers[0] = matrixBuffer;
		vsBuffers[1] = camera.GetConstantBuffer();
		...
		immediateContext->VSSetConstantBuffers(0, 1, &vsBuffers[0]);
		immediateContext->VSSetConstantBuffers(1, 1, &vsBuffers[1]);
...
void CameraMovement(CameraD3D11 camera, POINT p, RECT rect) {
